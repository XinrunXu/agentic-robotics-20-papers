...
SYSTEMS += '<p>EMERGE-Policy 把同一条思路推到信息平面：RoboOS 关心跨本体谁共享状态、PhyAgentOS 关心一次执行结束由谁验收，它则进一步规定感知、监控、验证与记忆整理各自在隔离上下文里处理，只把结构化结论交回主规划器。读它时可以沿用上面那张表继续追问：回传的结构化证据保留了什么、外部记忆文件在什么时候被整理、以及做出完成判定的那个 Sub Agent 是否真的独立于给出计划的那个模型。</p>'
...
SYSTEMS += refs([('RoboOS','2505.03673','roboos'),('PhyAgentOS','2607.16636','phyagentos'),('EMERGE-Policy','2608.29896','emerge')])
